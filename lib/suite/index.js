...
const DEFAULT_SUITE_DATA_DIR = path.join(__dirname, "..", "..", "data", "suite");
let suiteDataDir = DEFAULT_SUITE_DATA_DIR;

function setDataDirForTests(dir) {
  suiteDataDir = dir ? path.join(dir, "suite") : DEFAULT_SUITE_DATA_DIR;
}
...
function recordsFileFor(username) {
  const safe = String(username || "")
    ...
    .toLowerCase()
    .replace(/[^a-z0-9_.-]/g, "_");
  const usersDir = path.join(suiteDataDir, "users");
...
module.exports = {
  ...
  get SUITE_USERS_DIR() {
    return path.join(suiteDataDir, "users");
  },
  setDataDirForTests,
  productOf,
  isSuiteRequest,
