...
const { normalizeEntityType, showsOvernightTravel } = require("./profile");
...
function calcExpenseDeduction(entry, profile = {}) {
  ...
  const meals = mealCapsForYear(fy, salary, profile);
...
function summariseYear(records, profile = {}) {
  ...
  const meals = mealCapsForYear(fyFilter, annualSalary, profile);
  ...
  return {
    ...
    allowances: {
      ...
      travelKind: meals.travelKind || "ordinary_employee",
      occupationTravelKind: meals.occupationTravelKind || null,
      occupation: meals.occupation || null,
      workSnapshot: showsOvernightTravel(profile) ? null : buildWorkSnapshot(records, profile, fyFilter),
