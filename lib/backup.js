...
const dataDirMod = require("./data-dir");
...
function getDataDir() {
  return dataDirOverride || dataDirMod.getDataDir();
}
...
function countAccounts(dir = getDataDir()) {
  try {
...
function hasLedgerFiles(dir = getDataDir()) {
  const dirs = [path.join(dir, "users"), path.join(dir, "suite", "users")];
  for (const folder of dirs) {
    if (!fs.existsSync(folder)) continue;
    try {
      if (fs.readdirSync(folder).some((name) => name.endsWith(".json"))) return true;
    } catch {
      /* ignore */
    }
  }
...
function loadAccountUsers(dir = getDataDir()) {
  try {
    const raw = JSON.parse(fs.readFileSync(path.join(dir, "users.json"), "utf8"));
    return Object.values((raw && raw.users) || {});
  } catch {
    return [];
  }
}
...
/**
 * A wiped Render disk often still has the bootstrapped primary mod and no
 * driver ledgers. Treat that as empty so a leftover backup can restore profiles.
 */
function isBareBootstrapStore(dir = getDataDir()) {
  if (hasLedgerFiles(dir)) return false;
  const users = loadAccountUsers(dir);
  if (users.length === 0) return true;
  return users.length === 1 && Boolean(users[0] && users[0].isAdmin);
}

function envAutorestoreOn() {
  const raw = process.env.BACKUP_AUTORESTORE;
  if (raw == null || raw === "") return true;
  return !["0", "false", "no", "off"].includes(String(raw).toLowerCase());
}

/**
 * If a new deploy left accounts/ledgers empty (or only the primary mod) but a
 * local backup is still on the disk, restore the latest archive so profiles
 * are not wiped.
 */
async function restoreLatestIfStoreEmpty() {
  if (!envAutorestoreOn()) {
    return { restored: false, reason: "disabled" };
  }
  const accounts = countAccounts();
  const ledgers = hasLedgerFiles();
  if (!isBareBootstrapStore()) {
    return { restored: false, reason: "store-present", accounts, ledgers };
  }
  const backups = listBackups();
  if (!backups.length) return { restored: false, reason: "no-backup", accounts, ledgers };
  const latest = backups[0];
  const result = await restoreBackup(latest.id, { confirm: "RESTORE" });
  return {
    restored: true,
    id: latest.id,
    accounts: countAccounts(),
    safetyBackupId: result && result.safetyBackupId,
  };
}
