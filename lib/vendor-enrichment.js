...
const CANONICAL_VENDORS = [
  ...
  },
  {
    ...
    patterns: [/\bbp\b(?:\s+(?:truck\s*stop|servo|connect|outlet|service|express))?/i],
...
const SITE_NAME_REJECT_RE =
  /\b(rewards?|fleet|card|truck\s*stop|servo|connect|outlet|service|express|pty|ltd|limited|australia|group|retail|holdings?|tax|invoice|receipt|abn|tel|phone|gst|total|debit|credit|eftpos|approved|nab|visa|master)\b/i;

/**
 * Prefer "BP Archerfield" over bare "BP" when the site appears in the vendor
 * field or receipt header (without replacing independent names).
 */
function preferCanonicalSiteName(canonicalName, vendor, text) {
  const brand = String(canonicalName || "").trim();
  if (!brand) return null;
  const brandRe = brand.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
  ...
  for (const line of candidates) {
    ...
    const brandOut = /^bp$/i.test(m[1]) ? "BP" : m[1];
    const siteOut =
      /^[A-Z0-9\s'&-]+$/.test(site) && !/[a-z]/.test(site)
        ? site
            .toLowerCase()
            .split(/\s+/)
            ...
            .join(" ")
        : site;
    return `${brandOut} ${siteOut}`.replace(/\s+/g, " ").trim();
  }
  return null;
}
...
function memoryConflictsWithCanonical(memoryName, canonicalDisplayName) {
  ...
  const chainEntry =
    matchCanonicalVendorInText(display) || matchCanonicalVendorInText(display.split(/\s+/)[0] || "");
  const chainName = chainEntry ? chainEntry.name : display.split(/\s+/)[0];
  if (!chainName) return false;
  if (vendorsMatch(mem, chainName)) return false;
  const chainRe = new RegExp(
    `\\b${String(chainName).replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\\b`,
    "i"
  );
  if (chainRe.test(mem)) return false;
  return true;
}
...
function enrichOcrFromVendors(ocrResult, vendors, purpose = "expense") {
  ...
  if (known) {
    ...
    // unless a stronger chain/site canonical was just resolved that conflicts
    // (e.g. remembered "Rampage Retail" vs OCR "BP Archerfield").
    if (known.vendor.name) {
      ...
      const skipMemoryName =
        known.source === "abn" &&
        canonicalName &&
        memoryConflictsWithCanonical(known.vendor.name, canonicalName);
      if (
        !skipMemoryName &&
        (known.source === "abn" ||
          !prev ||
