...
function looksLikeJunkVendor(name) {
  ...
  // Alphanumeric OCR shreds: "XqR7", "S24", "4aZ9" — not real shop names.
  if (/\d/.test(s) && letters.length <= 4 && s.length <= 8 && !KNOWN_SHORT_BRANDS.test(s)) {
    if (!/\b(7[\s-]*eleven|7[\s-]*11)\b/i.test(s)) return true;
  }
...
function looksLikeOcrShredVendor(name) {
  ...
  // Clean bare chain names are fine (including short brands like BP).
  const exactCanonical = CANONICAL_VENDORS.some((e) => e.name.toLowerCase() === s.toLowerCase());
  if (exactCanonical) return false;
  // Title/ALL-caps multi-token thermal shreds with a 1–2 letter token
  // ("Woolworths Dra Wooiwoihs Gd", "Saree Gs Aran").
  if (words.length >= 2 && words.some((w) => w.replace(/[^A-Za-z]/g, "").length <= 2)) {
    if (!/^(bp|iga|otr|kfc|gyg|eg)$/i.test(words[0]) || words.length > 2) return true;
  }
