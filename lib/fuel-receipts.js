...
function saveReceiptImage(receiptId, dataUrl) {
  ...
  fs.writeFileSync(path.join(getDataDir(), relativePath), Buffer.from(match[2], "base64"));
  return relativePath;
}
...
function removeReceipt(store, id) {
  ensureLists(store);
  const row = findReceipt(store, id);
  if (!row) return false;
  if (row.imagePath) {
    const abs = receiptImageAbsPath(row.imagePath);
    if (abs && fs.existsSync(abs)) {
      try {
        fs.unlinkSync(abs);
      } catch {
        /* keep removing the row even if the file is already gone */
      }
    }
  }
  store.fuelReceipts = store.fuelReceipts.filter((r) => r.id !== id);
  return true;
}
