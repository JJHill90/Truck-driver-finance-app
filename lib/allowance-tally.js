/**
 * Daily overnight / travel allowance tallies (ATO TD stack for the selected FY).
 * Taxation Hub (truck drivers) band-1 example for 2025-26 (TD 2025/4 Table 5
 * meals + other-country accom/incidentals): meals $128 + overtime meal $38.65 +
 * accommodation $138 + incidentals $24.25 = $328.90 per day.
 * Go Taxation Suite uses Tables 1–3 capital-city amounts (Melbourne
 * representative) and excludes overtime from the overnight daily total.
 *
 * Caps come from summary.allowances.
 *
...
function isOrdinaryEmployeeTravel(allowances = {}) {
  return (
    allowances.travelKind === "ordinary_employee" ||
    allowances.dailyTravelTotal != null ||
    Boolean(allowances.workTravelMealsDaily)
  );
}

/** Segment definitions from summary.allowances (salary-band aware). */
function buildSegments(allowances = {}) {
  const meals = allowances.workTravelMealsDaily || allowances.truckDriverMealsDaily || {};
  ...
  const ordinary = isOrdinaryEmployeeTravel(allowances);
...
/** Daily grand-total allowance (meals once + accom + incidentals; OT only for truck stack). */
function dailyAllowanceTotal(allowances = {}) {
  if (allowances.dailyTravelTotal != null && Number.isFinite(Number(allowances.dailyTravelTotal))) {
