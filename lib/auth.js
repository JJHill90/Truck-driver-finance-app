...
function publicUser(user) {
  ...
  return {
    ...
    needsRecovery: (Number(user.failedLoginCount) || 0) >= MAX_FAILED_LOGINS,
  };
}
 ...
 * @returns {{ user: object|null, needsRecovery: boolean, failedLoginCount: number }}
 */
function attemptLogin(username, password) {
  ...
  const user = data.users[key];
  if (!user) {
    return { user: null, needsRecovery: false, failedLoginCount: 0 };
  }
