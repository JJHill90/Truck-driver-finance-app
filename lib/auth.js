  ...
  if (!user.isAdmin) {
    if (trialExpired(user)) {
      alerts.push({
        level: "warning",
        code: "trial_ended",
        message: `Your ${TRIAL_MONTHS}-month ${TRIAL_PRODUCT_LABEL} trial has ended. You’re on Free (15 uploads/month) — update to Pro (${PRO_PRICE_LABEL}) on the Profile tab for unlimited scans, PDF export and forecast.`,
      });
    } else if (trialEndingSoon(user)) {
      alerts.push({
        level: "info",
        code: "trial_ending",
        message: `Your ${TRIAL_PRODUCT_LABEL} trial ends soon. You can stay on Free afterwards, or start Pro (${PRO_PRICE_LABEL}) anytime from Profile → Plan — including from day one of signup.`,
      });
    }
  }
  return alerts;
