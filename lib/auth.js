...
// salted PBKDF2 hashes. Sessions persist to data/sessions.json so a new deploy
// does not log everyone out and make saved profiles look wiped.
//
...
function currentRoot() {
  return dataRootOverride || resolveDataDir();
}

function usersFile() {
  return path.join(currentRoot(), "users.json");
}
function sessionsFile() {
  return path.join(currentRoot(), "sessions.json");
}
function userRecordsDir() {
  return path.join(currentRoot(), "users");
}

function setDataDirForTests(dir) {
  dataRootOverride = dir || null;
  sessions.clear();
  sessionsHydrated = false;
  usersLoadUntrusted = false;
}
...
let usersLoadUntrusted = false;

function ensureDirs() {
  fs.mkdirSync(userRecordsDir(), { recursive: true });
}

function sessionUsernameOf(info) {
  if (!info) return "";
  return typeof info === "object" ? info.username : info;
}

function sessionCreatedAtOf(info) {
  if (info && typeof info === "object" && info.createdAt) return info.createdAt;
  return null;
}

function sessionStillValid(info) {
  const createdAt = sessionCreatedAtOf(info);
...
function persistSessionsNow() {
  ...
  for (const [token, info] of sessions.entries()) {
    const username = sessionUsernameOf(info);
    if (!token || !username || !sessionStillValid(info)) continue;
    payload[token] = {
      username: normaliseUsername(username),
      createdAt: sessionCreatedAtOf(info) || new Date().toISOString(),
    };
  }
  writeJsonAtomic(sessionsFile(), { sessions: payload });
}
...
function hydrateSessions() {
  ...
  try {
    ...
    const map = data && typeof data === "object" && data.sessions && typeof data.sessions === "object"
      ? data.sessions
    ...
    console.warn("session store unreadable:", err.message);
  }
}

/** Test helper: drop memory and re-read data/sessions.json (simulates a new build). */
...
function loadUsers() {
  ...
  try {
    if (fs.existsSync(usersFile())) {
      ...
      usersLoadUntrusted = true;
      console.error("users.json is present but invalid — not treating accounts as empty");
      return { users: {}, __untrusted: true };
    }
  } catch (err) {
    usersLoadUntrusted = true;
    console.error("users.json unreadable — refusing to overwrite accounts:", err.message);
    return { users: {}, __untrusted: true };
  }
  return { users: {} };
}
...
function saveUsers(data) {
  if (!data || data.__untrusted || usersLoadUntrusted) {
    console.error("Refusing to write users.json after a failed load (would wipe profiles)");
    return;
  }
  ensureDirs();
  const payload = { users: data.users || {} };
  writeJsonAtomic(usersFile(), payload);
}
...
function recordsFileFor(username) {
  ...
  return path.join(userRecordsDir(), `${safe}.json`);
}
...
function registerUser(username, password, presets = {}, email = "") {
  ...
  // Drivers start on Free. Primary mod is always Pro. Paid Pro / admin Pro+ come later.
  data.users[key] = user;
...
function destroySessionsForUser(username) {
  ...
  let changed = false;
  for (const [token, info] of sessions.entries()) {
    if (usernameKey(sessionUsernameOf(info)) === key) {
...
function getSessionUser(token) {
  ...
  if (!info) return null;
  if (!sessionStillValid(info)) {
    sessions.delete(token);
    persistSessionsNow();
    return null;
  }
  return sessionUsernameOf(info) || null;
}

function destroySession(token) {
  hydrateSessions();
  if (token && sessions.delete(token)) persistSessionsNow();
}
...
function accountAlerts(user, product) {
  const alerts = [];
  ...
  if (!user.isAdmin) {
    ...
    if (trialExpired(user)) {
      alerts.push({
        level: "warning",
        code: "trial_ended",
        message: `Your ${TRIAL_MONTHS}-month ${TRIAL_PRODUCT_LABEL} trial has ended. You’re on Free (${FREE_UPLOADS_PER_MONTH} uploads/month + ${FREE_ONSCREEN_REPORTS} on-screen EOFY report) — update to Pro (${prices.priceLabel}) on the Profile tab for unlimited scans, PDF export and forecast.`,
      });
    } else if (trialEndingSoon(user)) {
      alerts.push({
        level: "info",
        code: "trial_ending",
        message: `Your ${TRIAL_PRODUCT_LABEL} trial ends soon. Afterwards you keep Free (${FREE_UPLOADS_PER_MONTH} uploads/month + ${FREE_ONSCREEN_REPORTS} on-screen report), or start Pro (${prices.priceLabel}) anytime from Profile → Plan — including from day one of signup.`,
      });
    }
  }
  return alerts;
...
function getTrialOfferStatus(product) {
  return trialOfferStatus(product);
}
