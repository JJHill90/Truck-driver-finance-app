...
const { ensureBillingFields, isPro, assignFoundingTrial, foundingStatus } = require("./entitlements");
...
function publicUser(user) {
  ...
  return {
    ...
    foundingCohort: Boolean(user.foundingCohort),
    foundingSlot: user.foundingSlot || null,
...
function registerUser(username, password, presets = {}, email = "") {
  ...
  // Primary mod never consumes a founding trial slot.
  if (!makeAdmin) assignFoundingTrial(user, data);
  data.users[key] = user;
...
function getFoundingStatus() {
  return foundingStatus(loadUsers());
}
