...
function normalizeTruck(raw = {}) {
  ...
  const classCode = String(raw.classCode || "")
    .trim()
    .toUpperCase()
    .replace(/[^A-Z0-9]/g, "")
    .slice(0, 12) || null;
  const classConsumptionFactor = clamp(num(raw.classConsumptionFactor, 1), 0.85, 1.2);
  return {
    combinationId,
...
function consumptionLPer100km(truck, { band = "regional", fuelL, driverType } = {}) {
  ...
  const classFactor = spec.classConsumptionFactor || 1;
  const litres = (combo.baseLPer100 + trailerLift) * massFactor * terrain * duty * classFactor;
...
function describeEfficiency(truck, { band = "regional", driverType } = {}) {
  ...
  return {
    ...
    notes: [
      ...
      spec.classCode
        ? `Registered class ${spec.classCode} · ${spec.tankCapacityL} L tank (not a generic ${spec.combinationLabel} tank).`
        : `${spec.tankCapacityL} L tank from the work-vehicle combination default.`,
