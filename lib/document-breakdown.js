...
// Indicative ATO daily overnight (Living Away From Home) allowance for long-haul
// truck drivers — approximates the TD reasonable-amount total (~$132/day). Shown
// as a separate estimate line regardless of whether the payslip lists it (it is
// sometimes shown on a payslip as "Travel Allowance").
const DAILY_OVERNIGHT_ALLOWANCE = 132;
  ...
  if (isIncome) {
    ...
    if (driverType !== "local") {
      ...
      result.componentBreakdown.push({
        ...
        label: `ATO overnight allowance (${days} day${days === 1 ? "" : "s"} \u00d7 $${rate.toFixed(2)})`,
        amount,
        detected: false,
        note: "Living Away From Home Allowance for long-haul drivers — indicative ATO reasonable amount ($132/day), shown whether or not the payslip lists a travel allowance.",
      });
