...
const { pickBestExpenseTotal } = require("./expense-total");
const { parseMoney } = require("./receipt-ocr-money");
const { sgRateForYear, travelRatesForYear } = require("./historical-rates");
...
/** FY-aware truck-driver meal stack (breakfast + lunch + dinner). */
function truckMealsForFy(fy) {
  return travelRatesForYear(fy || getCurrentFinancialYear());
}

// Default export kept for older tests — current/latest published TD 2025/4 stack
// when no FY is supplied (128/day). Prefer truckMealsForFy(fy) for new call sites.
const DAILY_OVERNIGHT_ALLOWANCE = travelRatesForYear("2025-26").truckDriverMealsDailyTotal;
...
function parseAusDate(s) {
  // Use the shared AU parser (rejects invalid months and far-future OCR years).
  const iso = toIsoAusDate(s);
  if (!iso) return null;
...
function isTotalLabel(s) {
  return /\b(sale\s*total|grand\s*total|pay\s*total|total\s*due|amount\s*due|balance\s*due|amount\s*payable|invoice\s*total|total\s*payable|balance|total)\b/i.test(
    s
  );
}
...
function buildComponentBreakdown(ocr = {}, isIncome, financialYear) {
  ...
  // Prefer TOTAL / SALE TOTAL linked amounts from the receipt text over a weak
  // OCR amount (often a VISA/EFT tender or the first dollar on the page).
  const picked = pickBestExpenseTotal({
    ocrAmount: ocr.amount,
    rawText: ocr.rawTextPreview || ocr.rawText || "",
    lineItems: [...rawItems, ...baseItems],
  });
  let total = picked ? picked.amount : num(ocr.amount);
  const seen = new Set();
  ...
  for (const item of source) {
    ...
    if (isTotalLabel(low)) {
      // When text ranking already chose a total, keep it. Otherwise upgrade from
      // labeled total lines (never leave a smaller card tender locked in).
      if (!picked || picked.rank < 80) {
        if (!total || amount > total) total = amount;
      }
      continue;
    }
  ...
  if (total) {
    components.push({
      type: "total",
      label: picked && picked.rank >= 80 ? picked.label : "Grand total",
      amount: round2(total),
      detected: true,
    });
  }
...
function assessExpenseCompliance(ocr = {}, financialYear) {
  ...
  const checks = [];
  const travel = truckMealsForFy(fy);
  const meals = travel.truckDriverMeals;

  const mealCaps = {
    meals: travel.truckDriverMealsDailyTotal || COMBINED_MEALS_CAP,
    meals_breakfast: meals.breakfast.cap,
    meals_lunch: meals.lunch.cap,
    meals_dinner: meals.dinner.cap,
    overtime_meals: travel.overtimeMealCap,
  };
...
function analyzeScan(ocr = {}, purpose = "expense", profile = {}) {
  ...
  if (isIncome) {
    ...
    // Separate ATO LAFHA / driver daily meal allowance line for linehaul drivers.
    const driverType = profile.driverType || "long_haul";
    if (driverType !== "local") {
      const days = daysInPeriod(payPeriod);
      const travel = truckMealsForFy(fy);
      const rate = travel.truckDriverMealsDailyTotal;
      const amount = round2(days * rate);
      const meals = travel.truckDriverMeals;
      result.componentBreakdown.push({
        type: "overnight_allowance",
        label: `Living Away from Home / Driver Daily Allowance (${days} day${days === 1 ? "" : "s"} \u00d7 $${rate.toFixed(2)})`,
        amount,
        detected: false,
        note: `ATO reasonable daily meal amount for employee truck drivers (${travel.determination}): breakfast $${meals.breakfast.cap.toFixed(2)} + lunch $${meals.lunch.cap.toFixed(2)} + dinner $${meals.dinner.cap.toFixed(2)} = $${rate.toFixed(2)}/day for income year ${travel.incomeYear} (meals only, excludes accommodation). Claim actual spend up to this amount when you received a Travel / Living Away from Home (LAFHA) allowance and were away for work. Shown whether or not the payslip lists a travel allowance.`,
      });
      result.overnightAllowance = {
        perDay: rate,
        days,
        amount,
