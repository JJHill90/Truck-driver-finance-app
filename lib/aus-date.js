...
/**
 * Last-digit year confusions on thermal / low-contrast print (6↔8 is common).
 * Used when OCR yields an out-of-window or far-future year (18/08/2026 → 2028).
 */
const YEAR_LAST_DIGIT_CONFUSIONS = {
  0: [8, 6, 9],
  1: [7],
  3: [8, 5],
  5: [6, 3, 8],
...
function toIsoAusDate(input, now = new Date()) {
  ...
  if (input instanceof Date && !Number.isNaN(input.getTime())) {
    return isoFromPartsWithOcrRepairs(
      input.getFullYear(),
      input.getMonth() + 1,
      input.getDate(),
      now
  ...
  if (m) return isoFromPartsWithOcrRepairs(Number(m[1]), Number(m[2]), Number(m[3]), now);
  ...
  if (m) {
    let year = Number(m[3]);
    if (year < 100) {
      // Prefer 20xx even when out of window so year-digit repair can run
      // (guessDate does year += 2000 with no clamp — e.g. 28 → 2028).
      const expanded = expandTwoDigitYear(year, now);
      year = expanded != null ? expanded : 2000 + (((year % 100) + 100) % 100);
    }
    const day = Number(m[1]);
    const month = Number(m[2]);
    return isoFromPartsWithOcrRepairs(year, month, day, now);
  }
  ...
  if (m) {
    ...
    if (month) return isoFromPartsWithOcrRepairs(Number(m[3]), month, Number(m[1]), now);
  }
...
function yearLastDigitAlternatives(year) {
  const y = Number(year);
  if (!Number.isFinite(y) || y < 1000 || y > 9999) return [];
  const last = y % 10;
  const stem = y - last;
  return (YEAR_LAST_DIGIT_CONFUSIONS[last] || [])
    .map((d) => stem + d)
    .filter((alt) => alt !== y);
}

/**
 * When OCR misreads a year digit (2026 → 2028) so the date is out of the
 * plausible window or more than a day in the future, try last-digit
 * confusions and keep day/month.
 */
function maybeRepairConfusedYear(year, month, day, now = new Date()) {
  ...
  const directOk = isValidCalendarYmd(year, month, day, now);
  const directIso = directOk ? `${year}-${pad2(month)}-${pad2(day)}` : null;
  const directAge = directIso != null ? daysBetweenIso(directIso, todayIso) : null;
  // Needs repair when out-of-window, or >1 day in the future within the window.
  const needsRepair = !directIso || directAge < -1;
  if (!needsRepair) return null;

  let best = null;
  let bestScore = -Infinity;
  for (const altYear of yearLastDigitAlternatives(year)) {
    const alt = isoFromParts(altYear, month, day, now);
    if (!alt) continue;
    const age = daysBetweenIso(alt, todayIso);
    if (age < -1) continue;
    // Only accept nearby alternatives — do not jump to multi-year-old dates.
    if (age > 400) continue;
    // Prefer recent dockets (typical scan-soon-after-purchase).
    const score = age <= 120 ? 2000 - age : 800 - age / 5;
    if (score > bestScore) {
      bestScore = score;
      best = alt;
    }
...
/** Build ISO date, applying year then month OCR digit repairs when needed. */
function isoFromPartsWithOcrRepairs(year, month, day, now = new Date()) {
  const repairedYear = maybeRepairConfusedYear(year, month, day, now);
  let iso = repairedYear || isoFromParts(year, month, day, now);
  if (!iso) return null;
  const repairedMonth = maybeRepairConfusedMonth(iso, now);
  return repairedMonth || iso;
}
...
function extractBestDocumentDate(text, purpose = "expense", now = new Date()) {
  ...
  while ((match = DMY_GLOBAL_RE.exec(src)) != null) {
    ...
    const iso = isoFromPartsWithOcrRepairs(year, month, day, now);
    if (!iso) continue;

    const ctx = contextBefore(src, match.index);
    let rank = rankDateContext(ctx, purpose);
    rank += boostEftposTimestamp(src, iso);
    // Year/month repairs already applied in isoFromPartsWithOcrRepairs.
    if (iso !== isoFromParts(year, month, day, now)) rank += 20;
  ...
  while ((im = isoGlobal.exec(src)) != null) {
    const iso = isoFromPartsWithOcrRepairs(Number(im[1]), Number(im[2]), Number(im[3]), now);
    if (!iso) continue;
    const ctx = contextBefore(src, im.index);
    let rank = rankDateContext(ctx, purpose);
    if (iso !== isoFromParts(Number(im[1]), Number(im[2]), Number(im[3]), now)) rank += 20;
    const candidate = { date: iso, rank, label: ctx.slice(-40) };
    if (
      !best ||
      candidate.rank > best.rank ||
...
function resolveDocumentDate({
  ...
  // Final pass: repair OCR month/year digit confusion on the winning candidate
  // when the scan is clearly "today-ish" (e.g. 20/06/26 vs 20/08/26, or 2028 vs 2026).
  candidates.sort((a, b) => {
    if (b.rank !== a.rank) return b.rank - a.rank;
    return a.date < b.date ? 1 : a.date > b.date ? -1 : 0;
  });
  const top = candidates[0];
  const parts = String(top.date || "").split("-").map(Number);
  if (parts.length === 3) {
    const repaired = isoFromPartsWithOcrRepairs(parts[0], parts[1], parts[2], now);
    if (repaired) return repaired;
  }
  const repairedMonth = maybeRepairConfusedMonth(top.date, now);
  return repairedMonth || top.date;
}
...
module.exports = {
  ...
  maybeRepairConfusedYear,
  isoFromPartsWithOcrRepairs,
  DOCUMENT_YEAR_PAST,
