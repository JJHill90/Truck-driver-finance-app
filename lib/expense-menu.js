 ...
 * expose Food/Meals plus breakfast / lunch / dinner (ATO meal reasonable amounts).
...
const MEALS_META = {
  ...
  notes: "Combined food/meals when the meal type is unknown. Prefer breakfast/lunch/dinner when known. Shares the ATO daily meal pot with those lines.",
};

const MEAL_LABEL_OVERRIDES = {
...
function applyMenuPresentation(cat) {
  ...
  if (MEAL_LABEL_OVERRIDES[cat.id]) next.label = MEAL_LABEL_OVERRIDES[cat.id];
...
/**
 * Normalise expense category ids for save/OCR.
 * Breakfast / lunch / dinner stay as typed so Allowance caps can tally them;
 * unknown aliases still map onto `meals`.
 */
