 ...
 * ATO label (D1–D5), detailed transaction ledgers, LAFHA days (counter only),
 * tax estimate and ATO reasonable-amount / substantiation notes. First-party;
 * uses year-correct rates.
 */
...
} = require("./report-present");

function categoryLabel(id) {
  return presentExpenseLabel(id);
}
...
function buildReportPdf(report, records, fy) {
  ...
  function table(headers, rows, widths, aligns) {
    ...
    const drawRow = (cells, opts) => {
      ...
      doc
        .moveTo(left, y + rowH)
        .lineTo(left + widths.reduce((a, b) => a + b, 0), y + rowH)
        .lineWidth(0.5)
        .strokeColor(LINE)
        .stroke();
      doc.y = y + rowH;
  ...
  doc
    ...
    .fontSize(9.5)
    .fillColor(MUTED)
    .text("Tax return working paper \u2014 prepared for a registered tax agent");
  doc.moveDown(0.3);
  ...
  const profileRows = [
    ...
    { cells: ["ABN", d.abn || "\u2014"] },
    { cells: ["Driver type", d.driverType ? driverTypeLabel(d.driverType) : "\u2014"] },
    { cells: ["TFN supplied to employer", d.tfnSupplied ? "Yes" : "No"] },
    {
      cells: [
        "Salary band (travel allowance table)",
        s.profile && s.profile.salaryBand ? s.profile.salaryBand.replace("band", "Band ") : "\u2014",
      ],
    },
  ];
  ...
  incRows.push({
    cells: ["Total", money(s.income.grossTotal), money(s.income.assessableTotal)],
    bold: true,
  });
  table(["Income type", "Gross", "Assessable"], incRows, [contentWidth - 220, 110, 110], ["left", "right", "right"]);
  note(
    "Include assessable income at the relevant labels on your return (e.g. item 1 Salary or wages; item 2 Allowances, earnings, tips). Allowances shown as assessable only where a related deduction is claimed. Totals use taxable/gross income (not net pay)."
  );
  ...
  table(
    ["Category", "ATO schedule", "Items", "Deductible"],
    dedRows,
    [contentWidth - 250, 130, 45, 75],
    ["left", "left", "right", "right"]
  );
  note(
    "ATO deduction labels: D1 Work-related car; D2 Work-related travel; D3 Work-related clothing, laundry & dry-cleaning; D4 Self-education; D5 Other work-related expenses. Amounts are the work-related (deductible) portion after apportionment. Category names match Taxation Hub expense menus."
  );
  ...
  const expLedger = fyExpenses
    ...
    .map((e) => ({
      cells: [
        ...
        getCategoryMeta(e.category) && getCategoryMeta(e.category).atoSchedule
          ? String(getCategoryMeta(e.category).atoSchedule).split(" ")[0]
          : "\u2014",
        money(e.amount),
  ...
  const incLedger = fyIncome
    ...
    .sort((a, b) => String(a.date).localeCompare(String(b.date)))
    .map((i) => {
      const assessableBase = assessableIncomeAmount(i);
      const assessable = calcIncomeAssessability({ ...i, amount: assessableBase }).assessable;
      return {
        cells: [
          i.date || "",
          i.payer || i.entity || "\u2014",
          presentIncomeTypeLabel(i.type),
          money(numPositive(i.grossTotal) || assessableBase),
          money(assessable),
        ],
      };
    });
  ...
  // ---- LAFHA / Travel allowance days (counter only) --------------------
  const lafha = report.lafhaDays || {};
  heading("Living Away from Home (LAFHA) days");
  const lafhaRows = [
    { cells: ["Financial year", fyLabel(lafha.financialYear || fy)] },
    { cells: ["ATO determination (meal rate reference)", lafha.determination || "\u2014"] },
    { cells: ["Days in financial year", String(lafha.daysInFy != null ? lafha.daysInFy : "\u2014")] },
    { cells: ["FY days elapsed", String(lafha.daysElapsed != null ? lafha.daysElapsed : "\u2014")] },
    { cells: ["Travel / LAFHA days claimed", String(lafha.daysClaimed != null ? lafha.daysClaimed : 0)] },
    {
      cells: [
        "Projected EOFY Travel / LAFHA days",
        String(lafha.projectedYearEndDays != null ? lafha.projectedYearEndDays : "\u2014"),
      ],
    },
    {
      cells: [
        "Percent of FY days claimed",
        lafha.percentOfFyClaimed != null ? `${Number(lafha.percentOfFyClaimed).toFixed(1)}%` : "\u2014",
      ],
    },
  ];
  table(["Field", "Days / detail"], lafhaRows, [contentWidth - 150, 150], ["left", "right"]);
  const dayEntries = Array.isArray(lafha.entries) ? lafha.entries : [];
  if (dayEntries.length) {
    const entryRows = dayEntries.map((e) => ({
      cells: [e.date || "\u2014", e.label || "Payslip", String(e.days), e.source || "\u2014"],
    }));
    table(
      ["Date", "Source row", "Days", "How counted"],
      entryRows,
      [72, contentWidth - 222, 50, 100],
      ["left", "left", "right", "left"]
    );
  }
  note(
    "LAFHA section is a days counter only (not dollar totals). Days come from payslip Travel / Living Away from Home hours or days when present, otherwise from Travel/LAFHA $ \u00f7 the truck-driver meal rate for this income year. Confirm with payslips and your tax adviser."
  );
  ...
  note(
    `Tax computed using the resident individual rates and levies for FY ${fyLabel(t.ratesFinancialYear || fy)}. Excludes offsets (e.g. LITO), HELP/HECS and private health considerations.`
  );
  ...
  if (a.domesticTravelCaps) {
    capRows.push({
      cells: ["Domestic travel \u2013 accommodation (daily)", money(a.domesticTravelCaps.accommodation)],
    });
    capRows.push({
      cells: ["Domestic travel \u2013 incidentals (daily)", money(a.domesticTravelCaps.incidentals)],
    });
  }
  table(["Allowance", "Daily amount"], capRows, [contentWidth - 150, 150], ["left", "right"]);
  note(
    "Reasonable amounts (per the relevant year's Taxation Determination) are a substantiation exception under Subdivision 900-B of the ITAA 1997. You may claim only what you actually incurred; a travel diary / work (fatigue) diary supports the claim. Meal amounts are per meal and cannot be aggregated or transferred between meals."
  );
  ...
  doc
    ...
    .fontSize(8.5)
    .fillColor(INK)
    .text(`\u2022 ${sub.message || "Keep records for all expenses."}`, { width: contentWidth });
  doc.text(
    "\u2022 Written evidence (receipts/invoices) is required where total work-related deductions exceed $300.",
    { width: contentWidth }
  );
  doc.text("\u2022 Retain records for five years from the date you lodge your return.", {
    width: contentWidth,
  });
  doc.text(
    "\u2022 Reimbursed amounts are not deductible; salary-sacrificed / employer-provided items are excluded.",
    { width: contentWidth }
  );
  ...
  note(
    "This working paper is not a tax return and has not been lodged with the ATO. Provide it to your registered tax agent for review."
  );
  ...
  for (let i = 0; i < range.count; i++) {
    ...
    doc
      ...
      .fillColor(MUTED)
      .text(
        `Taxation Hub EOFY working paper \u2014 FY ${fyLabel(fy)} \u2014 page ${i + 1} of ${range.count}`,
        left,
        doc.page.height - 26,
