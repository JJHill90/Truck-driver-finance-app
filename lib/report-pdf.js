...
function buildReportPdf(report, records, fy) {
  ...
  doc.fontSize(8.5).fillColor(MUTED).text(`Generated ${new Date(report.generatedAt || Date.now()).toLocaleString("en-AU")}`);
  ...
  for (let i = 0; i < range.count; i++) {
    ...
    doc.font("Helvetica").fontSize(7.5).fillColor(MUTED).text(
      `Haulage EOFY working paper \u2014 FY ${fyLabel(fy)} \u2014 page ${i + 1} of ${range.count}`,
      left,
