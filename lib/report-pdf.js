...
const LINE = "#e4e7ec";
const ACCENT = "#b45309";
/** Soft slate-teal for vending-machine expense rows (noticeable, not bright). */
...
function buildReportPdf(report, records, fy) {
  ...
  const expLedger = fyExpenses
    ...
    .map((e) => {
      const flags = [];
      if (e.vendingMachine) flags.push("Vending");
      if (e.cashTransaction) flags.push("Cash");
      if (e.noReceipt) flags.push("No receipt");
      const row = {
        cells: [
          e.date || "",
          e.vendor || "\u2014",
  ...
  table(
    ["Date", "Vendor", "Category", "Sch.", "Amount", "Work", "Deductible", "Flags"],
    expLedger,
    [52, 84, 100, 36, 54, 30, 64, contentWidth - 420],
    ["left", "left", "left", "left", "right", "right", "right", "left"]
  );
  note("Rows shaded in soft slate-teal are vending-machine purchases (often no printed receipt) marked for spend reconciliation.");
