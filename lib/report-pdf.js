 ...
 * ATO label (D1–D5), category/vendor summary ledgers, tax estimate and ATO
...
const LINE = "#e4e7ec";
const ACCENT = "#b45309";
/** Soft slate-teal for categories that include vending-machine purchases. */
const VENDING_FILL = "#c5d4d8";
const VENDING_INK = "#2a4045";
...
function buildReportPdf(report, records, fy) {
  ...
  // Generic table: headers[], rows[{cells,bold,fill,ink}], widths[], aligns[]
  ...
  // ---- Expense ledger by category (compact) -----------------------------
  const expenseByCategory = summariseExpensesByCategory(records.expenses, fy);
  heading("Expense ledger (by category)");
  const expLedger = expenseByCategory.map((row) => {
    const flags = [];
    if (row.vendingCount) flags.push(`Vending ${row.vendingCount}`);
    if (row.cashCount) flags.push(`Cash ${row.cashCount}`);
    if (row.noReceiptCount) flags.push(`No receipt ${row.noReceiptCount}`);
    const out = {
      cells: [
        row.label,
        row.atoSchedule,
        String(row.count),
        money(row.amountTotal),
        money(row.deductibleTotal),
        flags.join(", ") || "\u2014",
      ],
    };
    if (row.vendingCount > 0) {
      out.fill = VENDING_FILL;
      out.ink = VENDING_INK;
    }
    return out;
  });
  if (!expLedger.length) {
    expLedger.push({ cells: ["No expense categories for this year", "", "", "", "", ""] });
  } else {
    const totAmt = expenseByCategory.reduce((a, r) => a + r.amountTotal, 0);
    const totDed = expenseByCategory.reduce((a, r) => a + r.deductibleTotal, 0);
    const totCount = expenseByCategory.reduce((a, r) => a + r.count, 0);
    expLedger.push({
      cells: ["Total", "", String(totCount), money(totAmt), money(totDed), ""],
      bold: true,
    });
  }
  table(
    ["Category", "Sch.", "Items", "Amount", "Deductible", "Flags"],
    expLedger,
    [contentWidth - 310, 42, 40, 70, 70, 88],
    ["left", "left", "right", "right", "right", "left"]
  );
  note("Category totals only (individual transactions are omitted to keep the working paper short). Soft slate-teal rows include one or more vending-machine purchases.");

  // ---- Income ledger by vendor + roaming total -------------------------
  const incomeByVendor = summariseIncomeByVendor(records.income, fy);
  heading("Income ledger (by vendor)");
  const incLedger = incomeByVendor.map((row) => ({
    cells: [
      row.vendor,
      String(row.count),
      money(row.grossTotal),
      money(row.assessableTotal),
      money(row.runningTotal),
    ],
  }));
  if (!incLedger.length) {
    incLedger.push({ cells: ["No income vendors for this year", "", "", "", ""] });
  } else {
    const last = incomeByVendor[incomeByVendor.length - 1];
    const totGross = incomeByVendor.reduce((a, r) => a + r.grossTotal, 0);
    const totAssess = incomeByVendor.reduce((a, r) => a + r.assessableTotal, 0);
    const totCount = incomeByVendor.reduce((a, r) => a + r.count, 0);
    incLedger.push({
      cells: ["Total", String(totCount), money(totGross), money(totAssess), money(last.runningTotal)],
      bold: true,
    });
  }
  table(
    ["Vendor / payer", "Entries", "Gross", "Assessable", "Running total"],
    incLedger,
    [contentWidth - 320, 50, 80, 90, 100],
    ["left", "right", "right", "right", "right"]
  );
  note("Income is grouped by vendor / payer. The running total is a roaming assessable total that grows as more income is added and the report is regenerated.");
