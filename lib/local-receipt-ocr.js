...
function extractMoneyFromText(text) {
  ...
  const moneyRe = /\$?\s*(\d{1,4}(?:,\d{3})*\.\d{2})\b/g;
  let match;
  while ((match = moneyRe.exec(src)) !== null) {
    const amount = parseMoney(match[1]);
    if (amount && amount < 100000) amounts.push(amount);
  }

  const labeled = [];
  const labelRe =
    /\b(GRAND\s*TOTAL|AMOUNT\s*DUE|BALANCE\s*DUE|TOTAL\s*DUE|EFT|VISA|MASTERCARD|MAESTRO|PAY\s*TOTAL|TOTAL(?!\s*INCLUDES)|SUBTOTAL|GST|TAX)\b[^\n\d$]{0,24}\$?\s*(\d{1,4}(?:,\d{3})*\.\d{2})/gi;
  ...
  const preferred = labeled.find((l) =>
    /^(GRAND TOTAL|AMOUNT DUE|BALANCE DUE|TOTAL DUE|PAY TOTAL|TOTAL|EFT|VISA|MASTERCARD)$/.test(
      l.label.replace(/\s+/g, " ")
    )
  );
  if (preferred) {
