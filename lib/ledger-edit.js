...
function updateExpense(records, id, patch = {}) {
  ...
  if (patch.cashTransaction !== undefined) entry.cashTransaction = Boolean(patch.cashTransaction);
  if (patch.noReceipt !== undefined) entry.noReceipt = Boolean(patch.noReceipt);
  if (patch.vendingMachine !== undefined) entry.vendingMachine = Boolean(patch.vendingMachine);
...
function updateIncome(records, id, patch = {}) {
  ...
  if (patch.travelAllowanceAmount !== undefined) {
    entry.travelAllowanceAmount =
      patch.travelAllowanceAmount == null || patch.travelAllowanceAmount === ""
        ? null
        : numOr(patch.travelAllowanceAmount, entry.travelAllowanceAmount ?? null);
  }
  if (patch.overnightDays !== undefined) {
    entry.overnightDays =
      patch.overnightDays == null || patch.overnightDays === ""
        ? null
        : Math.round(numOr(patch.overnightDays, entry.overnightDays ?? null));
  }
  if (patch.overnightDaysSource !== undefined) {
    entry.overnightDaysSource = patch.overnightDaysSource
      ? String(patch.overnightDaysSource)
      : null;
  }
  entry.updatedAt = new Date().toISOString();
  return entry;
