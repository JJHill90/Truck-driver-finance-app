 ...
 *   3. FormSubmit (support only) — posts to the developer inbox from the server
 *
 * Drivers never see FormSubmit activation. The first production send may email
 * support@godriverhub.com a one-time confirm link; later sends go through.
 */
let nodemailer = null;
try {
...
const DEFAULT_SUPPORT_EMAIL = "support@godriverhub.com";
...
function formSubmitActivationRequired(message) {
  return /activate|confirm your email|activation/i.test(String(message || ""));
}
...
async function sendMail({ to, subject, text, html, replyTo, attachments }) {
  ...
  if (resendConfigured()) {
    return sendViaResend({ to, subject, text, html, replyTo, attachments });
  }
...
/**
 * Server-side FormSubmit so the Support tab does not call it from the browser.
 * Activation (if still needed) goes to the developer inbox, not the driver.
 */
  ...
  try {
    const res = await fetch(endpoint, {
      ...
      body: JSON.stringify({
        ...
        _autoresponse:
          confirmationText ||
          buildSupportConfirmationText({ name, supportEmail: inbox }),
      }),
    });
    const data = await res.json().catch(() => ({}));
    const rawMessage = data && (data.message || data.error || "");
    if (formSubmitActivationRequired(rawMessage)) {
      console.warn("[mail:formsubmit] inbox activation still pending for", inbox);
      return {
        sent: false,
        channel: "formsubmit",
        activationRequired: true,
        error: String(rawMessage || "FormSubmit activation pending"),
      };
    }
    const ok =
      res.ok &&
      ...
      data.success !== "false";
    if (!ok) {
      const errMsg = String(rawMessage || `FormSubmit HTTP ${res.status}`);
      console.warn("[mail:formsubmit]", errMsg);
      return { sent: false, channel: "formsubmit", error: errMsg };
    }
...
function buildSupportNotificationText({ name, email, phone, message, username, priority }) {
  const fields = supportFieldLines({ name, email, phone, username });
  const body = String(message || "").trim() || "(empty message)";
  return [
    priority ? "PRIORITY — Driver Hub support request" : "Driver Hub — new support request",
    "====================================",
    "",
    "CONTACT DETAILS",
    "---------------",
 ...
 * }>}
 */
async function sendSupportEmail({ name, email, phone, message, username, to, priority }) {
  const inbox = supportInbox(to);
  const subject = `${priority ? "[PRIORITY] " : ""}Driver Hub support — from ${name}`;
  const text = buildSupportNotificationText({ name, email, phone, message, username, priority });
  const html = buildSupportNotificationHtml({ name, email, phone, message, username, priority });
  ...
  if (!toDev.sent) {
    delivered = await sendViaFormSubmit({
      ...
      username,
      message,
      confirmationText: buildSupportConfirmationText({ name, supportEmail: inbox }),
    });
  }
  if (!delivered.sent) {
    return {
      ...
      channel: delivered.channel || toDev.channel,
      to: inbox,
      error: delivered.error || toDev.error,
      preview: toDev.preview,
      activationRequired: Boolean(delivered.activationRequired),
    };
  }

  let confirmationSent = delivered.channel === "formsubmit";
  if (!confirmationSent) {
    try {
