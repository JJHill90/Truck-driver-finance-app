...
/**
 * Notify the support inbox about a contact-form submission.
 * @returns {Promise<{ sent: boolean, preview?: string, to: string }>}
 */
async function sendSupportEmail({ name, email, phone, message, username, to }) {
  const inbox = to || process.env.SUPPORT_EMAIL || "hilljj1990@gmail.com";
  ...
  const result = await sendMail({ to: inbox, subject, text, replyTo: email });
  return { ...result, to: inbox };
}
