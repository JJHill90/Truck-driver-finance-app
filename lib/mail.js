...
function mailFromAddress() {
  return (
    ...
    "DriverHub <onboarding@resend.dev>"
  );
}
...
function buildSupportNotificationHtml({ name, email, phone, message, username }) {
  ...
  return `<!DOCTYPE html>
<html lang="en">
<head><meta charset="utf-8" /><title>DriverHub support</title></head>
...
function buildSupportConfirmationText({ name, supportEmail }) {
  return [
    `Hi ${name},`,
    "",
    "Thanks for contacting DriverHub support.",
    ...
    <tr>
      <td align="center">
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:560px;background:#ffffff;border:1px solid #c5d0db;border-radius:12px;overflow:hidden;">
          <tr>
            <td style="background:#0b3d6e;padding:18px 22px;">
              <div style="font:700 18px/1.3 Arial,Helvetica,sans-serif;color:#ffffff;">DriverHub</div>
          ...
          <tr>
            <td style="padding:22px;font:400 15px/1.55 Arial,Helvetica,sans-serif;color:#1a2332;">
              ...
              <p style="margin:0 0 14px;">Thanks for contacting DriverHub support.</p>
              ...
              <p style="margin:0;color:#5a6b7d;">— DriverHub</p>
            </td>
          </tr>
        </table>
      </td>
...
async function sendSupportEmail({ name, email, phone, message, username, to }) {
  ...
  const subject = `DriverHub support — from ${name}`;
  ...
  try {
    const confirm = await sendMail({
      to: email,
      subject: "DriverHub — we received your support request",
...
async function sendRecoveryEmail({ to, username, resetUrl }) {
  const subject = "DriverHub — recover your username and password";
  const text = [
    "You requested to recover your DriverHub account.",
    "",
    `Username: ${username}`,
    "",
...
async function sendPasswordAgeEmail({ to, username, changeUrl }) {
  const subject = "DriverHub — please change your password";
  const text = [
    `Hi ${username},`,
    "",
    "Your DriverHub password is more than 90 days old.",
    "For security, please choose a new strong password:",
    changeUrl,
    "",
    "You can also change it any time on the Profile tab after signing in to FinanceHub.",
  ].join("\n");
  return sendMail({ to, subject, text });
}
