...
/**
 * Display labels layered over ato-standards DRIVER_TYPES (kept verbatim).
 * Internal ids (e.g. long_haul) stay stable for stored profiles + ATO rules.
 */
...
const DRIVER_ROLE_DEFAULTS = {
  ...
  long_haul: {
    annualSalary: 120000,
    notes: "Linehaul / interstate — typically HC–MC pay bands.",
  },
  owner_driver: {
    annualSalary: 130000,
...
function driverTypeLabel(id, fallback) {
  const key = String(id || "");
  if (DRIVER_TYPE_LABELS[key]) return DRIVER_TYPE_LABELS[key];
  return fallback || (DRIVER_TYPES[key] && DRIVER_TYPES[key].label) || key;
}

/** DRIVER_TYPES with UI label overrides for /standards. */
function presentDriverTypes() {
  const out = {};
  for (const [id, meta] of Object.entries(DRIVER_TYPES)) {
    out[id] = {
      ...meta,
      label: driverTypeLabel(id, meta.label),
    };
  }
  return out;
}
...
function listDriverRoleDefaults() {
  return Object.keys(DRIVER_ROLE_DEFAULTS).map((id) => {
    ...
    return {
      id,
      label: driverTypeLabel(id, meta.label || id),
...
function getDriverRoleDefaults(driverType) {
  ...
  return {
    driverType: key,
    label: driverTypeLabel(key, meta.label || key),
    description: meta.description || "",
    annualSalary: row.annualSalary,
    licenceClass,
    notes: row.notes,
