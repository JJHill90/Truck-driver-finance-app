...
/**
 * Restore a soft-deleted entry (owner or admin). Does not change reconcile flags.
 * Idempotent: an already-active entry returns ok (alreadyActive) so the gallery
 * “Restore to ledger” control succeeds when the banner was stale.
 */
function restoreEntry(records, type, id, { username } = {}) {
  const entry = findEntry(records, type, id);
  if (!entry) return { ok: false, error: "Entry not found.", code: "not_found" };
  if (!isDeleted(entry)) return { ok: true, entry, alreadyActive: true };
  const now = new Date().toISOString();
