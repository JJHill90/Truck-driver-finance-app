...
function addReceipt(records, payload) {
  const entry = {
    ...
    purpose: payload.purpose === "income" ? "income" : "expense",
...
function deleteReceiptFile(relativePath) {
...
/**
 * Remove a scanned photo/document and its receipt record.
 * Linked expense/income ledger rows are kept; photo links are cleared.
 */
function deleteReceipt(records, id) {
  const receipt = (records.receipts || []).find((r) => r.id === id);
  if (!receipt) return false;

  if (receipt.imagePath) deleteReceiptFile(receipt.imagePath);

  for (const e of records.expenses || []) {
    if (e.receiptId === id) e.receiptId = null;
  }
  for (const i of records.income || []) {
    if (i.receiptId === id) i.receiptId = null;
  }

  records.receipts = (records.receipts || []).filter((r) => r.id !== id);
  return true;
}
