/**
 * Accumulate overnight / travel-allowance days claimed in a financial year
...
/**
 * Resolve overnight days for one income row.
 * Prefer stored overnightDays; else travelAllowanceAmount ÷ meal rate;
...
function summariseOvernightDays(records = {}, profile = {}, financialYear) {
  ...
  return {
    ...
    entries,
    note: `${rates.determination}: overnight / LAFHA days are taken from the payslip Travel/Overnight hours or days counter when present, otherwise estimated from Travel/Overnight/LAFHA $ ÷ the truck-driver meal rate ($${round2(ratePerDay)}/day) for ${rates.incomeYear}. Use this as a planning snapshot of nights claimed versus days in the financial year — confirm with your payslips and tax adviser.`,
  };
}
