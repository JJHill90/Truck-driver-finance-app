...
function pickBestExpenseTotal({ ocrAmount = null, rawText = "", lineItems = [] } = {}) {
  ...
  function amountLooksLikePanInText(amount) {
    const v = round2(amount);
    if (!(v > 0)) return false;
    return String(rawText || "")
      .split(/\r?\n/)
      .some((line) => {
        if (!looksLikeCardPanFragment(line, v)) return false;
        const intPart = String(Math.floor(v));
        return new RegExp(String.raw`\b${intPart}(?:\.\d{1,2})?\b`).test(line);
      });
  }
  ...
  for (const item of lineItems || []) {
    ...
    if (amountLooksLikePanInText(amount)) continue;
  ...
  function addCandidate(base, { amount = base.amount, source = "labeled", repaired = false } = {}) {
    const v = round2(amount);
    if (!(v > 0)) return;
    if (amountLooksLikePanInText(v)) return;
  ...
  if (fallback > 0) {
    ...
    const panLine = String(rawText || "")
      .split(/\r?\n/)
      .find((line) => {
        if (!looksLikeCardPanFragment(line, fallback)) return false;
        const amounts = moneyAmountsIn(line);
        if (amounts.some((a) => Math.abs(a - fallback) < 0.005)) return true;
        // OCR often drops a trailing zero: 5822.1 vs 5822.10
        const intPart = String(Math.floor(fallback));
        return new RegExp(String.raw`\b${intPart}(?:\.\d{1,2})?\b`).test(line);
      });
    if (panLine) {
...
function refineExpenseDetectedTotals(detectedTotals, ocrResult = {}, components = []) {
  ...
  // Strip card-PAN-looking rows even when we cannot find a better total.
  for (let i = out.length - 1; i >= 0; i--) {
    const row = out[i];
    const rawHint = `${row.label || ""} ${ocrResult.rawText || ocrResult.rawTextPreview || ""}`;
    const lineHit = String(ocrResult.rawText || ocrResult.rawTextPreview || "")
      .split(/\r?\n/)
      .find((line) => {
        if (!looksLikeCardPanFragment(line, row.amount)) return false;
        const intPart = String(Math.floor(Number(row.amount) || 0));
        return new RegExp(String.raw`\b${intPart}(?:\.\d{1,2})?\b`).test(line);
      });
    if (lineHit || looksLikeCardPanFragment(rawHint, row.amount)) {
      out.splice(i, 1);
    }
  }
  ...
  if (!best) {
    if (out.length) {
      ...
      // Prefer a remaining mid-size total over nothing; leave empty if all were PAN.
