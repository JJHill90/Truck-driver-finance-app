...
function presentFuelVehicle(vehicle, extras = {}) {
  if (!vehicle) return null;
  const row = normalizeFuelVehicle(vehicle);
  if (!row) return null;
  const range = describeClassRange(row, {
    combinationId: extras.combinationId || row.typicalCombination || "rigid",
    driverType: extras.driverType || "long_haul",
  });
